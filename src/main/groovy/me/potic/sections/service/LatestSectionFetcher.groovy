...
class LatestSectionFetcher implements SectionFetcher {
    ...
    List<Card> fetch(User user, Map fetchCardsRequest) {
        log.debug "fetching cards for user ${user} with request ${fetchCardsRequest}"
        ...
        try {
            ...
            log.error "fetching cards for user ${user} with request ${fetchCardsRequest} failed: $e.message", e
            throw new RuntimeException("fetching cards for user ${user} with request ${fetchCardsRequest} failed", e)
        }
    }
}
